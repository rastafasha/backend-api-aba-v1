...
function create_REF($row, $ref, $segTer, $compEleSep)
{
    ...
    if ($ref == '9F') {       /***Required when Referring Provider is sent (REF*DN)*/
        $REF[1] = $ref;
...
function create_x12_837_file($res, $segTer, $eleDataSep, $compEleSep)
{
    ...
    foreach ($res as $row) {
        ...
        $file_data .= create_ST($row, $segTer, $compEleSep) . PHP_EOL;
        ++$loopcounter;
        $file_data .= create_BHT($row, $segTer, $compEleSep) . PHP_EOL;
        ++$loopcounter;

        ## SUBMITTER NAME-1000A
        $file_data .= create_NM1($row, 'SUBMITTER', $segTer, $compEleSep) . PHP_EOL;
        ++$loopcounter;

        if (!empty($row['submitter_telephone'])) {
            ## SUBMITTER EDI CONTACT INFORMATION-1000A
            $file_data .= create_PER($row, 'TE', $segTer, $compEleSep) . PHP_EOL;
            ++$loopcounter;
        }
        if (!empty($row['submitter_email'])) {
            ## SUBMITTER EDI CONTACT INFORMATION-1000A
            $file_data .= create_PER($row, 'EM', $segTer, $compEleSep) . PHP_EOL;
            ++$loopcounter;
        }
        if (!empty($row['submitter_fax'])) {
            ## SUBMITTER EDI CONTACT INFORMATION-1000A
            $file_data .= create_PER($row, 'FX', $segTer, $compEleSep) . PHP_EOL;
            ++$loopcounter;
        }

        ## RECEIVER NAME-1000B
        $file_data .= create_NM1($row, 'RC', $segTer, $compEleSep) . PHP_EOL;
        ++$loopcounter;

        ## 2010AA BILLING PROVIDER HL LOOP
        $file_data .= create_HL($row, 1, $segTer, $compEleSep) . PHP_EOL;
        ++$loopcounter;
        ## BILLING PROVIDER NAME 2010AA
        $file_data .= create_NM1($row, 'BP', $segTer, $compEleSep) . PHP_EOL;
        ++$loopcounter;
        ## BILLING PROVIDER ADDRESS
        $file_data .= create_N3($row, 'BP', $segTer, $compEleSep) . PHP_EOL;
        ++$loopcounter;
        ## BILLING PROVIDER CITY, STATE, ZIP CODE
        $file_data .= create_N4($row, 'BP', $segTer, $compEleSep) . PHP_EOL;
        ++$loopcounter;

        ## Situational PRV segment for provider taxonomy code for Medicaid. only applicable for MC
        if (!empty($row['biller_tax_code'])) {
            $file_data .= create_PRV($row, $segTer, $compEleSep) . PHP_EOL;
            ++$loopcounter;
        }
        ...
        if (!empty($row['billing_provider_federal_taxid'])) {
            $file_data .= create_REF($row, 'EI', $segTer, $compEleSep) . PHP_EOL;
            ++$loopcounter;
        } elseif (!empty($row['billing_provider_pin'])) {
            $file_data .= create_REF($row, 'SY', $segTer, $compEleSep) . PHP_EOL;
            ++$loopcounter;
        }

        ## 2010AB PAY-TO PROVIDER
        ## PAY-TO PROVIDER NAME 2010AB
        $file_data .= create_NM1($row, '87', $segTer, $compEleSep) . PHP_EOL;
        ++$loopcounter;
        ...
        $file_data .= create_HL($row, 2, $segTer, $compEleSep) . PHP_EOL;
        ++$loopcounter;
        ## SUBSCRIBER INFORMATION 2000B
        $file_data .= create_SBR($row, 'PR', $segTer, $compEleSep) . PHP_EOL;
        ++$loopcounter;
        ## SUBSCRIBER SECONDARY IDENTIFICATION 2010BA
        $file_data .= create_NM1($row, 'IL', $segTer, $compEleSep) . PHP_EOL;
        ++$loopcounter;
        ## SUBSCRIBER ADDRESS 2010BA
        $file_data .= create_N3($row, 'SB', $segTer, $compEleSep) . PHP_EOL;
        ++$loopcounter;
        ## SUBSCRIBER ADDRESS 2010BA
        $file_data .= create_N4($row, 'SB', $segTer, $compEleSep) . PHP_EOL;
        ++$loopcounter;
        ## SUBSCRIBER DEMOGRAPHIC INFORMATION 2010BA
        $file_data .= create_DMG($row, $segTer, $compEleSep) . PHP_EOL;
        ++$loopcounter;

        ## PAYER NAME Loop 2010BB
        $file_data .= create_NM1($row, 'PR', $segTer, $compEleSep) . PHP_EOL;
        ++$loopcounter;
        ## PAYER ADDRESS 2010BB
        $file_data .= create_N3($row, 'PR', $segTer, $compEleSep) . PHP_EOL;
        ++$loopcounter;
        ## PAYER CITY, STATE, ZIP CODE
        $file_data .= create_N4($row, 'PR', $segTer, $compEleSep) . PHP_EOL;
        ++$loopcounter;

        ## CLAIM INFORMATION 2300
        $file_data .= create_CLM($row, $segTer, $compEleSep) . PHP_EOL;
        ++$loopcounter;

        ## DATE ONSET OF CURRENT ILLNESS OR SYMPTOM
        $file_data .= create_DTP($row, '431', $segTer, $compEleSep) . PHP_EOL;
/* S */ ++$loopcounter;
        ## DATE - INITIAL TREATMENT DATE 2300
        $file_data .= create_DTP($row, '454', $segTer, $compEleSep) . PHP_EOL;
/* S */ ++$loopcounter;
        ## DATE - LAST SEEN DATE 2300
        $file_data .= create_DTP($row, '304', $segTer, $compEleSep) . PHP_EOL;
        ++$loopcounter;

        if (!empty($row['patient_discharge_date'])) {
            ## DATE OF DISCHARGE 2300
            $file_data .= create_DTP($row, '096', $segTer, $compEleSep) . PHP_EOL;
/* S */ ++$loopcounter;
        }

        if (!empty($row['patient_admission_date'])) {
            ## DATE OF ADMISSION 2300
            $file_data .= create_DTP($row, '435', $segTer, $compEleSep) . PHP_EOL;
/* S */ ++$loopcounter;
        }
...
/* S */ ++$loopcounter;
        ...
        if (!empty($row['original_claim_number'])) {
            ...
            $file_data .= create_REF($row, 'X4', $segTer, $compEleSep) . PHP_EOL;
            ++$loopcounter;
        }

        if (!empty($row['patient_mrn'])) {
            ## MEDICAL RECORD NUMBER 2300
            $file_data .= create_REF($row, 'EA', $segTer, $compEleSep) . PHP_EOL;
            ++$loopcounter;
        }

        if (!empty($row['claim_notes'])) {
            ## CLAIM NOTE 2300
            $file_data .= create_NTE($row, $segTer, $compEleSep) . PHP_EOL;
            ++$loopcounter;
        }
        ...
        $file_data .= create_NM1($row, 'DN', $segTer, $compEleSep) . PHP_EOL;
        ++$loopcounter;/* S */
        ...
        if (!empty($row['subscriber_secondary_group_number'])) {
            $file_data .= create_SBR($row, 'OT', $segTer, $compEleSep) . PHP_EOL;
            ++$loopcounter;
            ...
            if (!empty($row['secondary_payer_name'])) {
                $file_data .= create_NM1($row, 'PR2', $segTer, $compEleSep) . PHP_EOL;
                ++$loopcounter;
            }
            if (!empty($row['secondary_payer_street'])) {
                $file_data .= create_N3($row, 'PR2', $segTer, $compEleSep) . PHP_EOL;
                ++$loopcounter;
            }
            if (!empty($row['secondary_payer_city'])) {
                $file_data .= create_N4($row, 'PR2', $segTer, $compEleSep) . PHP_EOL;
                ++$loopcounter;
            }
        }
        ...
        if (sizeof($row['procedure_codes'])) {
            ...
            foreach ($row['procedure_codes'] as $proc_items) {
                ...
                $file_data .= create_LX($row, $loopindex, $segTer, $compEleSep) . PHP_EOL;
                ++$loopcounter;
                ## PROFESSIONAL SERVICE 2400
                $file_data .= create_SV1($row, $proc_items, $segTer, $compEleSep) . PHP_EOL;
                ++$loopcounter;
                $file_data .= create_DTP($row, 472, $segTer, $compEleSep, $proc_items['dos']) . PHP_EOL;
                ++$loopcounter;
