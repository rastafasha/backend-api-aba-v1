...
class Edi837pHelper
{
    ...
    public static function create_ISA($row, $segTer, $compEleSep)
    {
    ...
    public static function create_GS($row, $segTer, $compEleSep)
    {
    ...
    public static function create_GE($row, $segTer, $compEleSep)
    {
    ...
    public static function create_IEA($row, $segTer, $compEleSep)
    {
    ...
    public static function create_ST($row, $segTer, $compEleSep)
    {
    ...
    public static function create_BHT($row, $segTer, $compEleSep)
    {
    ...
    public static function create_NM1($row, $nm1Cast, $segTer, $compEleSep)
    {
        ...
        if ($nm1Cast == 'RC') {/* RECIEVER*/
        ...
        } elseif ($nm1Cast == '87') {               /* BILLING PROVIDER NAME PAY TO PROVIDER BLOCK*/
        ...
        } elseif ($nm1Cast == 'BP') {               /* BILLING PROVIDER NAME*/
            ...
            $NM1[8] = "XX";
            $NM1[9] = $row['billing_provider_npi'];     /* Patient Doctors NPI*/
        } elseif ($nm1Cast == 'SUBMITTER') {                /*Patient - Subscriber Details */
        ...
        } elseif ($nm1Cast == 'SB') {               /*Patient - Subscriber Details */
            $NM1[1] = "41";                     /* Insured or Subscriber */
    ...
    public static function create_PER($row, $type, $segTer, $compEleSep)
    {
    ...
    public static function create_HL($row, $nHlCounter, $segTer, $compEleSep)
    {
    ...
    public static function create_PRV($row, $segTer, $compEleSep)
    {
    ...
    public static function create_N3($row, $ref, $segTer, $compEleSep)
    {
        ...
        } elseif ($ref == 'BP') { //Billing Provider
        ...
        } elseif ($ref == 'FA') { //Facility Hospital Details
    ...
    public static function create_N4($row, $ref, $segTer, $compEleSep)
    {
        ...
        if ($ref == 'SB') { // Subscriber Details
            ...
            //$N4[4] = ""                                   /* payer country code */
        } elseif ($ref == 'FA') { //Facility or Hospital
    ...
    public static function create_REF($row, $ref, $segTer, $compEleSep)
    {
        ...
        if ($ref == 'F8') {       /*(Required when CLM05-03 indicates replacement or void to a previously adjudicated claim */
            $REF[1] = $ref;
            $REF[2] = $row['original_claim_number'];
        }
        if ($ref == 'X4') {       /*Facilities performing CLIA covered Laboratory services*/
            $REF[1] = $ref;
            $REF[2] = $row['original_claim_number'];     /*Clinical Laboratory Improvement Amendment Number*/
        }
        if ($ref == 'EA') {       /*CTUAL MEDICAL RECORD OF THE PATIENT*/
            $REF[1] = $ref;
    ...
    public static function create_SBR($row, $ref, $segTer, $compEleSep)
    {
        ...
        $SBR[2] = static::translate_relationship($row['subscriber_relationship']);
    ...
    */
    ...
    public static function create_DMG($row, $segTer, $compEleSep)
    {
    ...
    public static function create_CLM($row, $segTer, $compEleSep)
    {
    ...
    */
    ...
    public static function create_DTP($row, $ref, $segTer, $compEleSep, $dos = '')
    {
    ...
    public static function create_AMT($row, $segTer, $compEleSep)
    {
    ...
    /*Additional notes */
    public static function create_NTE($row, $segTer, $compEleSep)
    {
    ...
    public static function create_HI($row, $primary, $segTer, $compEleSep)
    {
    ...
    public static function create_OI($row, $segTer, $compEleSep)
    {
    ...
    public static function create_LX($row, $loopcount, $segTer, $compEleSep)
    {
    ...
    public static function create_SV1($row, $items, $segTer, $compEleSep)
    {
    ...
    public static function create_SE($row, $segmentcount, $segTer, $compEleSep)
    {
    ...
    public static function create_x12_837_file($res, $segTer, $eleDataSep, $compEleSep)
    {
        ...
        foreach ($res as $row) {
            ...
            $file_data .= static::create_ISA($row, $segTer, $compEleSep) . PHP_EOL;

            $file_data .= static::create_GS($row, $segTer, $compEleSep) . PHP_EOL;
            ## ST Count Should start from here including ST
            $file_data .= static::create_ST($row, $segTer, $compEleSep) . PHP_EOL;
            ++$loopcounter;
            $file_data .= static::create_BHT($row, $segTer, $compEleSep) . PHP_EOL;
            ++$loopcounter;

            ## SUBMITTER NAME-1000A
            $file_data .= static::create_NM1($row, 'SUBMITTER', $segTer, $compEleSep) . PHP_EOL;
            ++$loopcounter;

            if (!empty($row['submitter_telephone'])) {
                ## SUBMITTER EDI CONTACT INFORMATION-1000A
                $file_data .= static::create_PER($row, 'TE', $segTer, $compEleSep) . PHP_EOL;
                ++$loopcounter;
            }
            if (!empty($row['submitter_email'])) {
                ## SUBMITTER EDI CONTACT INFORMATION-1000A
                $file_data .= static::create_PER($row, 'EM', $segTer, $compEleSep) . PHP_EOL;
                ++$loopcounter;
            }
            if (!empty($row['submitter_fax'])) {
                ## SUBMITTER EDI CONTACT INFORMATION-1000A
                $file_data .= static::create_PER($row, 'FX', $segTer, $compEleSep) . PHP_EOL;
                ++$loopcounter;
            }

            ## RECEIVER NAME-1000B
            $file_data .= static::create_NM1($row, 'RC', $segTer, $compEleSep) . PHP_EOL;
            ++$loopcounter;

            ## 2010AA BILLING PROVIDER HL LOOP
            $file_data .= static::create_HL($row, 1, $segTer, $compEleSep) . PHP_EOL;
            ++$loopcounter;
            ## BILLING PROVIDER NAME 2010AA
            $file_data .= static::create_NM1($row, 'BP', $segTer, $compEleSep) . PHP_EOL;
            ++$loopcounter;
            ## BILLING PROVIDER ADDRESS
            $file_data .= static::create_N3($row, 'BP', $segTer, $compEleSep) . PHP_EOL;
            ++$loopcounter;
            ## BILLING PROVIDER CITY, STATE, ZIP CODE
            $file_data .= static::create_N4($row, 'BP', $segTer, $compEleSep) . PHP_EOL;
            ++$loopcounter;

            ## Situational PRV segment for provider taxonomy code for Medicaid. only applicable for MC
            if (!empty($row['biller_tax_code'])) {
                $file_data .= static::create_PRV($row, $segTer, $compEleSep) . PHP_EOL;
                ++$loopcounter;
            }
            ...
            if (!empty($row['billing_provider_federal_taxid'])) {
                $file_data .= static::create_REF($row, 'EI', $segTer, $compEleSep) . PHP_EOL;
                ++$loopcounter;
            } elseif (!empty($row['billing_provider_pin'])) {
                $file_data .= static::create_REF($row, 'SY', $segTer, $compEleSep) . PHP_EOL;
                ++$loopcounter;
            }

            ## 2010AB PAY-TO PROVIDER
            ## PAY-TO PROVIDER NAME 2010AB
            $file_data .= static::create_NM1($row, '87', $segTer, $compEleSep) . PHP_EOL;
            ++$loopcounter;
            ...
            $file_data .= static::create_HL($row, 2, $segTer, $compEleSep) . PHP_EOL;
            ++$loopcounter;
            ## SUBSCRIBER INFORMATION 2000B
            $file_data .= static::create_SBR($row, 'PR', $segTer, $compEleSep) . PHP_EOL;
            ++$loopcounter;
            ## SUBSCRIBER SECONDARY IDENTIFICATION 2010BA
            $file_data .= static::create_NM1($row, 'IL', $segTer, $compEleSep) . PHP_EOL;
            ++$loopcounter;
            ## SUBSCRIBER ADDRESS 2010BA
            $file_data .= static::create_N3($row, 'SB', $segTer, $compEleSep) . PHP_EOL;
            ++$loopcounter;
            ## SUBSCRIBER ADDRESS 2010BA
            $file_data .= static::create_N4($row, 'SB', $segTer, $compEleSep) . PHP_EOL;
            ++$loopcounter;
            ## SUBSCRIBER DEMOGRAPHIC INFORMATION 2010BA
            $file_data .= static::create_DMG($row, $segTer, $compEleSep) . PHP_EOL;
            ++$loopcounter;

            ## PAYER NAME Loop 2010BB
            $file_data .= static::create_NM1($row, 'PR', $segTer, $compEleSep) . PHP_EOL;
            ++$loopcounter;
            ## PAYER ADDRESS 2010BB
            $file_data .= static::create_N3($row, 'PR', $segTer, $compEleSep) . PHP_EOL;
            ++$loopcounter;
            ## PAYER CITY, STATE, ZIP CODE
            $file_data .= static::create_N4($row, 'PR', $segTer, $compEleSep) . PHP_EOL;
            ++$loopcounter;

            ## CLAIM INFORMATION 2300
            $file_data .= static::create_CLM($row, $segTer, $compEleSep) . PHP_EOL;
            ++$loopcounter;

            ## DATE ONSET OF CURRENT ILLNESS OR SYMPTOM
            $file_data .= static::create_DTP($row, '431', $segTer, $compEleSep) . PHP_EOL;
            /* S */
            ++$loopcounter;
            ## DATE - INITIAL TREATMENT DATE 2300
            $file_data .= static::create_DTP($row, '454', $segTer, $compEleSep) . PHP_EOL;
            /* S */
            ++$loopcounter;
            ## DATE - LAST SEEN DATE 2300
            $file_data .= static::create_DTP($row, '304', $segTer, $compEleSep) . PHP_EOL;
            ++$loopcounter;

            if (!empty($row['patient_discharge_date'])) {
                ## DATE OF DISCHARGE 2300
                $file_data .= static::create_DTP($row, '096', $segTer, $compEleSep) . PHP_EOL;
                /* S */
                ++$loopcounter;
            }
            ...
            if (!empty($row['patient_admission_date'])) {
                ...
                $file_data .= static::create_DTP($row, '435', $segTer, $compEleSep) . PHP_EOL;
                /* S */
                ++$loopcounter;
            }
            ...
            if (!empty($row['patient_paid_amt'])) {
                ## PATIENT AMOUNT PAID 2300
                $file_data .= static::create_AMT($row, $segTer, $compEleSep) . PHP_EOL;
                ++$loopcounter;
            }

            if (!empty($row['referral_number'])) {
                ## REFERRAL NUMBER 2300 **Required when Referring Provider is sent (REF*DN)
                $file_data .= static::create_REF($row, '9F', $segTer, $compEleSep) . PHP_EOL;
                /* S */
                ++$loopcounter;
            }
            ...
            if (!empty($row['prior_auth_code'])) {
                ...
                $file_data .= static::create_REF($row, 'G1', $segTer, $compEleSep) . PHP_EOL;
                ++$loopcounter;
            }

            if ($row['claim_type'] != 1) {
                ## PAYER CLAIM CONTROL NUMBER 2300 (Required when CLM05-03 indicates replacement or void
                $file_data .= static::create_REF($row, 'F8', $segTer, $compEleSep) . PHP_EOL;
                ++$loopcounter;
            }
            ...
            if (!empty($row['original_claim_number'])) {
                ...
                $file_data .= static::create_REF($row, 'X4', $segTer, $compEleSep) . PHP_EOL;
                ++$loopcounter;
            }

            if (!empty($row['patient_mrn'])) {
                ## MEDICAL RECORD NUMBER 2300
                $file_data .= static::create_REF($row, 'EA', $segTer, $compEleSep) . PHP_EOL;
                ++$loopcounter;
            }

            if (!empty($row['claim_notes'])) {
                ## CLAIM NOTE 2300
                $file_data .= static::create_NTE($row, $segTer, $compEleSep) . PHP_EOL;
                ++$loopcounter;
            }
            ## HEALTH CARE DIAGNOSIS CODE 2300
            $file_data .= static::create_HI($row, 1, $segTer, $compEleSep) . PHP_EOL;
            ++$loopcounter;
            /* Primary Problem code ICD 9 or ICD 10*/

            if (sizeof($row['other_diag_list'])) {
                $file_data .= static::create_HI($row, 2, $segTer, $compEleSep) . PHP_EOL;
                ++$loopcounter; /* secondart type code ICD 9 or ICD 10*/
            }

            ## REFERRING PROVIDER NAME 2310A
            $file_data .= static::create_NM1($row, 'DN', $segTer, $compEleSep) . PHP_EOL;
            ++$loopcounter;/* S */
            ...
            if (!empty($row['subscriber_secondary_group_number'])) {
                $file_data .= static::create_SBR($row, 'OT', $segTer, $compEleSep) . PHP_EOL;
                ++$loopcounter;

                ## Other Insurance Coverage Information
                //$file_data .= static::create_OI($row,$segTer,$compEleSep); ++$loopcounter;
                ...
                if (!empty($row['secondary_payer_name'])) {
                    $file_data .= static::create_NM1($row, 'PR2', $segTer, $compEleSep) . PHP_EOL;
                    ++$loopcounter;
                }
                if (!empty($row['secondary_payer_street'])) {
                    $file_data .= static::create_N3($row, 'PR2', $segTer, $compEleSep) . PHP_EOL;
                    ++$loopcounter;
                }
                if (!empty($row['secondary_payer_city'])) {
                    $file_data .= static::create_N4($row, 'PR2', $segTer, $compEleSep) . PHP_EOL;
                    ++$loopcounter;
                }
            }
            ...
            if (sizeof($row['procedure_codes'])) {
                ...
                foreach ($row['procedure_codes'] as $proc_items) {
                    ...
                    $file_data .= static::create_LX($row, $loopindex, $segTer, $compEleSep) . PHP_EOL;
                    ++$loopcounter;
                    ## PROFESSIONAL SERVICE 2400
                    $file_data .= static::create_SV1($row, $proc_items, $segTer, $compEleSep) . PHP_EOL;
                    ++$loopcounter;
                    $file_data .= static::create_DTP($row, 472, $segTer, $compEleSep, $proc_items['dos']) . PHP_EOL;
                    ++$loopcounter;
            ...
            ++$loopcounter;
            $file_data .= static::create_SE($row, $loopcounter, $segTer, $compEleSep) . PHP_EOL;
            $file_data .= static::create_GE($row, $segTer, $compEleSep) . PHP_EOL;
            $file_data .= static::create_IEA($row, $segTer, $compEleSep) . PHP_EOL;
        }
